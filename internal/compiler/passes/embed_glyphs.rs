        ...
        .map(|pixel_size| {
            let glyph_data = character_map
                ...
                .map(|CharacterMapEntry { code_point, .. }| {
                    ...
                    let limit_threshold = std::env::var("SLINT_LIMIT_GLYPHS_THRESHOLD").ok().and_then(|val| val.parse::<i16>().ok()).unwrap_or(125);
                    let limit_chars = std::env::var("SLINT_LIMIT_GLYPHS_CHARS").unwrap_or_else(|_| "0123456789., ".to_string());
                    let is_allowed = !(*pixel_size > limit_threshold) || limit_chars.contains(*code_point);
                    if !is_allowed {
                        return BitmapGlyph {
                            x: 0,
                            y: 0,
